...
export const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[var(--bg-primary)]/80 backdrop-blur-xl border-b border-[var(--border-card)]' : 'bg-transparent'}`}>
      <div className="w-full px-6 md:px-12 flex justify-between items-center h-20">
        <a href="#" className="text-2xl font-bold text-violet-400 hover:text-violet-300 transition-colors tracking-tight">
          D15
        </a>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors text-sm font-medium">
              {link.name}
            </a>
          ))}
        ...
        <div className="md:hidden absolute top-20 left-0 w-full bg-[var(--bg-primary)]/95 backdrop-blur-xl border-b border-[var(--border-card)] p-6 flex flex-col gap-6">
          ...
          <div className="flex gap-6 mt-2">
             {resumeData.personal.social.map((social) => (
                <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
                  <social.icon size={24} />
                </a>
             ))}
          </div>
